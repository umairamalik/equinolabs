...
import { Home, User, Star, Mail } from 'lucide-react';
...
const MobileNav: React.FC<MobileNavProps> = ({ currentPage, onNavigate }) => {
  const navItems = [
    ...
    // { id: SectionId.TESTIMONIALS, label: 'Testimonials', icon: Star },
  ...
  return (
    <div className="fixed bottom-0 left-0 w-full bg-brand-black/95 backdrop-blur-lg border-t border-gray-900 md:hidden z-[9999] px-2 pb-safe">
      <div className="flex justify-between items-center h-16">
        {navItems.map((item) => (
          ...
          >
            <item.icon 
              size={20} 
              className="text-gray-400 focus:text-brand-red active:text-brand-red"
            />
